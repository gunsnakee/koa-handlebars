    ...
    return new Promise((resolve, reject) => {
        ...
        readDir(dir, (err, result) => {
            ...
            Promise.all(partials).then(templates => {
                ...
                templates
                    .map(item => {
                      let relativePath = path.relative(partialPath,item.name).replace(/\\/g,'/')
                      let name = path.dirname(relativePath).indexOf('./') == 0 ? path.basename(relativePath, o.extension) : `${path.dirname(relativePath)}/${path.basename(relativePath, o.extension)}`
                        return Object.assign(item, {
                          name
                        })
                    })
                    .forEach(item => {
                      debug('partials [%s] will be compile', item.name)
                      Handlebars.registerPartial(item.name, item.body)
                    })
